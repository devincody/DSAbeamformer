...
int main(){
	...
	float *beam_out = new float[N_FREQUENCIES*N_BEAMS*N_OUTPUTS_PER_GEMM];
	...
	gpuErrchk(cudaHostRegister(beam_out, N_FREQUENCIES*N_BEAMS*N_OUTPUTS_PER_GEMM*sizeof(float), cudaHostRegisterPortable)); //need pinned memory
		...
		gpuErrchk(cudaHostRegister(out_dedispersed, N_BEAMS*sizeof(float), cudaHostRegisterPortable));
	...
	gpuErrchk(cudaMalloc(&d_out,  N_BEAMS*N_FREQUENCIES*N_STREAMS * sizeof(float)));			// array for detected, averaged data
	...
	gpuErrchk(cudaStreamCreate(&HtoD_stream));
	...
	while (!observation_complete){
		...
		if (blocks_analyzed < blocks_transfered){
			...
			for (int st = 0; st < N_STREAMS; st++){
				...
				expand_input<<<1000, 32, 0, stream[st]>>>(&d_data[N_BYTES_PRE_EXPANSION_PER_GEMM*(N_GEMMS_PER_BLOCK*(blocks_transfered%N_BLOCKS_on_GPU) + timeSlice[st])],
				...
				detect_sum<<<N_FREQUENCIES * N_OUTPUTS_PER_GEMM, N_BEAMS, 0, stream[st]>>>(&d_C[N_CX_OUT_PER_GEMM*st], d_out);

				gpuErrchk(cudaMemcpyAsync(beam_out, 
										  d_out, N_OUTPUTS_PER_GEMM*N_FREQUENCIES*N_BEAMS*sizeof(float), 
	...
	gpuErrchk(cudaStreamDestroy(HtoD_stream));
