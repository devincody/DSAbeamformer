...
__global__
void detect_sum(cuComplex *input, int n_avg, float *output){
	/*
	...
	*/
	...
	int n_beams = blockDim.x;	 	// should be = N_BEAMS
	int n_freq = gridDim.y;   		// should be = N_FREQUENCIES
	int n_batch = gridDim.x;		// should be = N_OUTPUTS_PER_GEMM = N_AVERAGING * N_POL

	int batching_idx = blockIdx.x;	// index along batching dimension
	int freq_idx = blockIdx.y;		// index along frequency dimension
	int beam_idx = threadIdx.x;		// index along beam dimension
	...
	int input_idx  	 = freq_idx * n_batch * n_avg * n_beams; 	// deal with frequency component first
	input_idx 		+= batching_idx * n_avg * n_beams;  		// deal with the indexing for each output next
																// Each thread starts on the first time-pol index 
																// (b/c we iterate over all time-pol indicies in for loop below)
	input_idx 		+= beam_idx;							   	// start each thread on the first element of each beam

	int output_idx = batching_idx * n_freq * n_beams + freq_idx * n_beams + beam_idx;
	...
	for (int i = 0; i < n_avg; i++){
		...
		shmem[beam_idx] += in.x*in.x;// + in.y*in.y;
